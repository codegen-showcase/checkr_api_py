...
def test_list_200_success_required_only():
    ...
    try:
        pydantic.TypeAdapter(models.UsersListResponse).validate_python(response)
...
def test_list_200_success_all_params():
    ...
    response = client.users.list(page=123.45, per_page=123.45)
    try:
        pydantic.TypeAdapter(models.UsersListResponse).validate_python(response)
        is_valid_response_schema = True
    except pydantic.ValidationError:
...
async def test_await_list_200_success_all_params():
    ...
    response = await client.users.list(page=123.45, per_page=123.45)
    try:
        pydantic.TypeAdapter(models.UsersListResponse).validate_python(response)
        is_valid_response_schema = True
    except pydantic.ValidationError:
