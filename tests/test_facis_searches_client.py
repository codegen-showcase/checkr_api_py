...
def test_get_200_success_all_params():
    ...
    response = client.facis_searches.get(id="string")
    try:
        pydantic.TypeAdapter(models.FacisSearch).validate_python(response)
...
async def test_await_get_200_success_all_params():
    ...
    response = await client.facis_searches.get(id="string")
    try:
        pydantic.TypeAdapter(models.FacisSearch).validate_python(response)
        is_valid_response_schema = True
    except pydantic.ValidationError:
