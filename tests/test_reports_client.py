...
def test_update_200_success_all_params():
    """Tests a POST request to the /reports/{id} endpoint.
    ...
    try:
        pydantic.TypeAdapter(models.Report).validate_python(response)
...
async def test_await_update_200_success_all_params():
    ...
    response = await client.reports.update(
        id="string", adjudication="engaged", package="driver_pro"
    )
    try:
        pydantic.TypeAdapter(models.Report).validate_python(response)
...
def test_create_200_success_all_params():
    """Tests a POST request to the /reports endpoint.
    ...
    response = client.reports.create(
        ...
        self_disclosures=[
            {
                "date": "2019-11-01",
                "description": "I made a mistake and grew from this experience.",
                "location": {"country": "US", "county": "BOULDER", "state": "CO"},
                "offense_category": "Insurance Fraud",
                "offense_level": "Misdemeanor",
                "sentence": "24 months probation",
                "time_served": "20 months probation",
            }
        ],
        tags=["string"],
...
async def test_await_create_200_success_all_params():
    ...
    response = await client.reports.create(
        candidate_id="e44aa283528e6fde7d542194",
        package="string",
        node="string",
        self_disclosures=[
...
def test_get_200_success_required_only():
    ...
    response = client.reports.get(id="string", include="candidate")
    try:
        pydantic.TypeAdapter(models.Report).validate_python(response)
        is_valid_response_schema = True
    except pydantic.ValidationError:
...
async def test_await_get_200_success_required_only():
    ...
    response = await client.reports.get(id="string", include="candidate")
    try:
        pydantic.TypeAdapter(models.Report).validate_python(response)
        is_valid_response_schema = True
    except pydantic.ValidationError:
