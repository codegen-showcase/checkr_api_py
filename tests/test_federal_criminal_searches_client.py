...
def test_get_200_success_required_only():
    """Tests a GET request to the /federal_criminal_searches/{id} endpoint.
    ...
    Response : models.FederalCriminalSearch
    ...
    response = client.federal_criminal_searches.get(id="string", exclude="district")
    try:
        pydantic.TypeAdapter(models.FederalCriminalSearch).validate_python(response)
...
async def test_await_get_200_success_required_only():
    ...
    response = await client.federal_criminal_searches.get(
        id="string", exclude="district"
    )
    try:
        pydantic.TypeAdapter(models.FederalCriminalSearch).validate_python(response)
        is_valid_response_schema = True
