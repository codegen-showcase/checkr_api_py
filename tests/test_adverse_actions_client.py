...
def test_get_200_success_all_params():
    """Tests a GET request to the /adverse_actions/{id} endpoint.
    ...
    response = client.adverse_actions.get(id="string")
    try:
...
async def test_await_get_200_success_all_params():
    ...
    response = await client.adverse_actions.get(id="string")
    try:
        pydantic.TypeAdapter(models.AdverseAction).validate_python(response)
        is_valid_response_schema = True
    except pydantic.ValidationError:
...
def test_delete_200_success_all_params():
    """Tests a DELETE request to the /adverse_actions/{id} endpoint.
    ...
    response = client.adverse_actions.delete(id="string")
    try:
        pydantic.TypeAdapter(models.AdverseAction).validate_python(response)
        is_valid_response_schema = True
    except pydantic.ValidationError:
...
async def test_await_delete_200_success_all_params():
    ...
    response = await client.adverse_actions.delete(id="string")
    try:
        pydantic.TypeAdapter(models.AdverseAction).validate_python(response)
        is_valid_response_schema = True
    except pydantic.ValidationError:
