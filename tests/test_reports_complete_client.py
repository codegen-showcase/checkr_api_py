...
def test_create_200_success_all_params():
    """Tests a POST request to the /reports/{id}/complete endpoint.
    ...
    response = client.reports.complete.create(id="string")
    try:
...
async def test_await_create_200_success_all_params():
    ...
    response = await client.reports.complete.create(id="string")
    try:
        pydantic.TypeAdapter(models.Report).validate_python(response)
        is_valid_response_schema = True
    except pydantic.ValidationError:
