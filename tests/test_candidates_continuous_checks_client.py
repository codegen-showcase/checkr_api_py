...
def test_create_201_success_all_params():
    """Tests a POST request to the /candidates/{candidate_id}/continuous_checks endpoint.
    ...
    response = client.candidates.continuous_checks.create(
        candidate_id="string",
        type_="criminal",
        node="string",
...
async def test_await_create_201_success_all_params():
    ...
    response = await client.candidates.continuous_checks.create(
        candidate_id="string",
        type_="criminal",
        node="string",
        work_locations=[{"city": "San Francisco", "country": "US", "state": "CA"}],
...
def test_list_200_success_all_params():
    """Tests a GET request to the /candidates/{candidate_id}/continuous_checks endpoint.
    ...
    Response : models.CandidatesContinuousChecksListResponse
    ...
    response = client.candidates.continuous_checks.list(candidate_id="string")
    try:
...
async def test_await_list_200_success_all_params():
    ...
    response = await client.candidates.continuous_checks.list(candidate_id="string")
    try:
        pydantic.TypeAdapter(
            models.CandidatesContinuousChecksListResponse
        ).validate_python(response)
